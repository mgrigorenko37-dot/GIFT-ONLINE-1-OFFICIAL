...
async function main() {
  ...
  const runWorker = (name: string, shouldCrashOnI?: number) => {
    return new Promise((resolve) => {
      const env = Object.assign(
        {},
        process.env,
        shouldCrashOnI !== undefined ? { CRASH_ON_I: String(shouldCrashOnI) } : {}
      );
  ...
  const pending = await pool.query(
    `SELECT COUNT(*) as c FROM te_outbox_events WHERE user_id = $1 AND status = 'test_pending'`,
    [user]
  );
  const published = await pool.query(
    `SELECT COUNT(*) as c FROM te_outbox_events WHERE user_id = $1 AND status = 'test_published'`,
    [user]
  );
  console.log(`\n--- DB VERIFICATION ---`);
  ...
  const pendingCrash = await pool.query(
    `SELECT COUNT(*) as c FROM te_outbox_events WHERE user_id = $1 AND status = 'test_pending'`,
    [user]
  );
  console.log(
    `Pending events after crash: ${pendingCrash.rows[0].c} (expected 5, because transaction rollbacks)`
  );

  console.log('Spawning Outbox Worker D to recover');
  await runWorker('D');

  const pendingRecover = await pool.query(
    `SELECT COUNT(*) as c FROM te_outbox_events WHERE user_id = $1 AND status = 'test_pending'`,
    [user]
  );
  const publishedRecover = await pool.query(
    `SELECT COUNT(*) as c FROM te_outbox_events WHERE user_id = $1 AND status = 'test_published'`,
    [user]
  );
