...
(async () => {
  ...
  await pool.query('ALTER TABLE te_executions DROP CONSTRAINT IF EXISTS te_executions_user_id_instrument_key_key').catch(e => {});
  ...
  for(const c of execCols) {
    await pool.query(`ALTER TABLE te_executions ADD COLUMN IF NOT EXISTS ${c.split(' ')[0]} ${c.split(' ').slice(1).join(' ')}`).catch(e => {});
  }
