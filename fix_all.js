...
`;
  ...
  const placeOrderRegex = /(public async placeOrder\([\s\S]*?try \{\n      await client.query\('BEGIN'\);\n)([\s\S]*?)(const now = Date\.now\(\);)/;
  code = code.replace(placeOrderRegex, (match, p1, p2, p3) => {
    // replace inside p2
    let newP2 = p2;
    // remove existing lock of balance
    newP2 = newP2.replace(/await client\.query\(\s*'SELECT available_balance FROM te_balances WHERE user_id = \$1 AND currency = \$2 FOR UPDATE',\s*\[order\.userId, order\.collateralCurrency\]\s*\);/g, '');
    newP2 = newP2.replace(/await client\.query\(\s*'SELECT available_balance FROM te_balances WHERE user_id = \$1 AND currency = \$2 FOR UPDATE',\s*\[orderData\.userId, collateralCurrency\]\s*\);/g, '');
    
    // add lockMarginResources after currencies are defined
    if (!newP2.includes('this.lockMarginResources')) {
      const injectPos = newP2.indexOf('const collateralCurrency = instrumentConfig.collateralCurrency;') + 'const collateralCurrency = instrumentConfig.collateralCurrency;'.length;
      newP2 = newP2.substring(0, injectPos) + '\n      await this.lockMarginResources(client, orderData.userId, collateralCurrency);\n' + newP2.substring(injectPos);
    }
...
`;
  ...
  code = code.replace(/await client\.query\(\s*'SELECT available_balance FROM te_balances WHERE user_id = \$1 AND currency = \$2 FOR UPDATE',\s*\[order\.userId, order\.collateralCurrency\]\s*\);/g, '');
  ...
  //   await client.query('BEGIN');
  //   const execCheck = ...
  const execStartRegex = /(public async executeTrade[\s\S]*?try \{\n      await client.query\('BEGIN'\);\n)([\s\S]*?)(const orderRes = await client\.query\(\s*'SELECT \* FROM te_orders WHERE order_id = \$1 FOR UPDATE',\s*\[orderId\]\s*\);)/;
  code = code.replace(execStartRegex, (match, p1, p2, p3) => {
    let newP2 = p2;
    if (!newP2.includes('initialOrderRes')) {
      const execCheckMatch = newP2.match(/const execCheck[\s\S]*?if \(options\?\.source && options\?\.externalExecutionId\) \{[\s\S]*?return null;\n\s*\}\n\s*\}/);
...
`;
  ...
  const balResRegex = /const balRes = await client\.query\(\s*'SELECT available_balance, locked_balance, realized_pnl, total_fees FROM te_balances WHERE user_id = \$1 AND currency = \$2 FOR UPDATE',\s*\[order\.userId, currency\]\s*\);/g;
  code = code.replace(balResRegex, `const balRes = await client.query('SELECT available_balance, locked_balance, realized_pnl, total_fees FROM te_balances WHERE user_id = $1 AND currency = $2', [order.userId, currency]);`);
  ...
  code = code.replace(/order\.feeCurrency,\s+'REJECTED',/g, "order.feeCurrency,\n              order.pnlCurrency || order.settlementCurrency,\n              'REJECTED',");
