...
const beforeMain = code.slice(0, mainStart);
      ...
      <main className='gx-main trade-wrapper'>
        <style>{\`
          ...
          .trade-wrapper .mkt-panel {
            position: absolute; top: 100%; left: 16px; width: 300px;
        ...
        <div className="topbar">
          <div className="brand" onClick={() => navigate('/market')}>
            ...
            <div className="crumbs">Markets <span>›</span> <b>{activeGift.name.toUpperCase().replace(/[^A-Z0-9]/g, '_')}</b></div>
          </div>
        ...
        <div className={\`mkt-strip \${mktPanelOpen ? 'open' : ''}\`} id="mktStrip">
          <div className="mkt-current" onClick={() => setMktPanelOpen(!mktPanelOpen)}>
            ...
            <span className="pp mono">{recentTrades.length > 0 ? formatGX(recentTrades[0].price) : formatGX(activeGift.floor)}</span>
            <span className={\`pc \${activeGift.change >= 0 ? 'chg-up' : 'chg-down'}\`}>
        ...
        <div className="layout">
          <div className="col">
            <div className="price-head">
              <div className="asset-price mono">{recentTrades.length > 0 ? formatGX(recentTrades[0].price) : formatGX(activeGift.floor)}</div>
              <span className={\`chg-tag \${activeGift.change >= 0 ? 'chg-up' : 'chg-down'}\`}>{activeGift.change >= 0 ? '+' : ''}{activeGift.change}%</span>
              <div className="stats-inline">
                <span className="stat-i">Макс<b>{(activeGift.floor * 1.05).toFixed(2)}</b></span>
                <span className="stat-i">Мин<b>{(activeGift.floor * 0.95).toFixed(2)}</b></span>
            ...
            <div className="orders-tabs">
              <div className={\`ot \${activityTab === 'open' ? 'active' : ''}\`} onClick={() => setActivityTab('open')}>Открытые ордера ({openOrders.length})</div>
              <div className={\`ot \${activityTab === 'history' ? 'active' : ''}\`} onClick={() => setActivityTab('history')}>История</div>
            </div>
            <div className="orders-empty">
              {openOrders.length === 0 ? 'Нет открытых ордеров' : openOrders.map(o => (
          ...
          <div className="col">
            ...
            <div className="ob-body">
              ...
              <div className="ob-mid">
                <span className="p">{recentTrades.length > 0 ? formatGX(recentTrades[0].price) : formatGX(activeGift.floor)}</span>
          ...
          <div className="col">
            <div className="panel-inner">
              ...
              <div className="summary">
                ...
                <div className="sum-row"><span>Итого</span><b>{formatGX((Number(price) || activeGift.floor) * (Number(amount) || 0))} TON</b></div>
              </div>

              <button className={\`cta \${side}\`} onClick={() => submitOrder(side)}>
...
fs.writeFileSync('src/screens/GXTerminal/GXTerminalScreen.tsx', beforeMain + newMain + afterMain);
