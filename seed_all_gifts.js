...
const GIFTS_CATALOG = [
  ...
  { name: "Strawberries Box", stars: 749 }, // wait, typo in original? { name: "Strawberries Box", stars: 749 }
...
const FIXED_GIFTS = GIFTS_CATALOG.map(g => typeof g.stars === 'number' ? g : { name: g.name, stars: 749 });
  ...
  try {
    ...
    await client.query(`
      CREATE TABLE IF NOT EXISTS gift_assets (
        ...
        base_price_ton NUMERIC NOT NULL,
      ...
      CREATE INDEX IF NOT EXISTS idx_candles_history_lookup ON candles_history (asset_id, timeframe, timestamp);
    `);
    
    await client.query('TRUNCATE TABLE gift_assets CASCADE');

    let assetIdCounter = 1;
    for (const gift of FIXED_GIFTS) {
      const basePriceTon = gift.stars * 0.002;
      ...
      for (let i = 0; i < 3; i++) {
        ...
        const symbol = `${symbolBase}_${model.toUpperCase()}_${backdrop.toUpperCase()}`;
        
        await client.query(`
          INSERT INTO gift_assets (id, symbol, name, stars_price, base_price_ton, model, backdrop, pattern)
          VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
          ON CONFLICT (symbol) DO NOTHING
        `, [assetIdCounter++, symbol, gift.name, gift.stars, basePriceTon, model, backdrop, pattern]);
      }
    }

    await client.query('COMMIT');
    console.log('Successfully seeded 115 gifts with 3 variants each.');
  } catch (err) {
