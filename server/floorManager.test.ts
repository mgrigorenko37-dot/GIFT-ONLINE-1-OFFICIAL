...
describe('Stage 10: Telegram Gifts Floor Price System Scenarios', () => {
  test('Runs Stage 10 Floor Manager Scenarios', () => {
    ...
    // 2. Cancellation
    {
      ...
      assert(
        floor.floorPrice === '20',
        'Test 2: Floor price increases to 20 after cancelling lowest listing'
      );
    ...
    // 4. Expiration (status = "expired")
    {
      ...
      assert(
        floor.floorPrice === '14',
        'Test 4: Floor price updates to 14 after lowest item expired'
      );
    ...
    // 6. No active listings (listedCount = 0, floorPrice = null)
    {
      ...
      assert(
        floorEmpty.floorPrice === null,
        'Test 6: Floor price must be null when no active listings exist'
      );
      assert(
        floorEmpty.listedCount === 0,
        'Test 6: Listed count must be 0 when no active listings exist'
      );
      ...
      assert(
        floorCancelled.floorPrice === null,
        'Test 6: Floor price must revert to null when all listings cancelled'
      );
      assert(
        floorCancelled.listedCount === 0,
        'Test 6: Listed count must be 0 when all listings cancelled'
      );

      console.log(
        '✓ Test 6 passed: No active listings yields floorPrice = null and listedCount = 0'
      );
    }
    ...
    // 8. Different instrumentKeys (collections / models / backdrops)
    {
      ...
      addListing({
        listingId: 'l1',
        instrumentKey: 'coll1:model1:bg1:TON',
        price: '10.0',
        currency: 'TON',
      });
      addListing({
        listingId: 'l2',
        instrumentKey: 'coll1:model2:bg1:TON',
        price: '20.0',
        currency: 'TON',
      });
    ...
    // 9. Floor changes do NOT change OHLCV sales candles
    {
      ...
      assert(
        candlesAfter.length === 0,
        'Test 9: Floor price changes must NOT create or modify OHLCV candles'
      );
    ...
    // 10. Independent processing of sale and floor_update
    {
      ...
      assert(
        ...
        'Test 10: Floor price becomes null after single listing sold'
      );
