...
export async function matchOrder(newOrder: Order) {
  ...
  try {
    ...
    `, [newOrder.user_id, newOrder.asset_id, newOrder.type, newOrder.side, newOrder.type === 'MARKET' ? 0 : newOrder.price, newOrder.amount]);
    ...
    const queryParams = newOrder.type === 'LIMIT' 
    ...
    for (const maker of makerOrders) {
      ...
      if (newOrder.side === 'BUY') {
        ...
        // SELL
        await updateBalance(client, newOrder.user_id, quoteCurrency, (matchAmount * makerPrice) * 0.998); // -0.2% fee
...
async function updateBalance(client: PoolClient, userId: string, assetId: string, amountChange: number) {
