...
// 1. Single sale
{
  ...
  addListing({ listingId: 'lst-1', instrumentKey: 'durov-cap:all:all:TON', price: '15.0', currency: 'TON' });
  ...
  assert(stats.priceChangePercent === '0', 'Test 1: priceChangePercent is 0 for single sale baseline');
...
// 2. Multiple sales & weighted average
{
  ...
  assert(stats.averageSalePrice === '17.5', 'Test 2: weighted averageSalePrice is 70 / 4 = 17.5');
  assert(stats.priceChange === '10', 'Test 2: priceChange is 20 - 10 = 10');
  assert(stats.priceChangePercent === '100', 'Test 2: priceChangePercent is (10 / 10) * 100 = 100%');
...
// 3. No sales
{
  ...
  assert(stats.lastSalePrice === null, 'Test 3: lastSalePrice must be null when no sales exist');
...
// 5. Currency isolation (TON vs STARS)
{
  ...
  const starsStats = getMarketStats({ instrumentKey: 'durov-cap:all:all:STARS', currency: 'STARS' });
...
// 7. Baseline Price Change calculation
{
  ...
  assert(stats.priceChangePercent === '50', 'Test 7: priceChangePercent is (25 / 50) * 100 = 50%');

  console.log('✓ Test 7 passed: Price change baseline (first completed sale in range) is accurate');
}
...
// 8. Late adjustment / Out-of-order sale insertion
{
  ...
  assert(stats.lastSalePrice === '20', 'Test 8: lastSalePrice remains 20 (chronologically latest eventTime)');
  assert(stats.lastSaleTime === 1710000060000, 'Test 8: lastSaleTime is 1710000060000');
  assert(stats.priceChange === '10', 'Test 8: priceChange is 20 - 10 = 10 after chronological sorting');
