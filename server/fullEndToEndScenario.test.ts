...
import { addListing, getFloorPrice, clearFloorState } from './floorManager';
...
describe('Stage 16: End-to-End Scenario & Integration Tests', () => {
  test('Runs full E2E scenario', () => {
    console.log(
      '=== Running Stage 16: Complete End-to-End User Scenario & Integration Verification ==='
    );
    ...
    // 1. Canonical instrumentKey construction & parsing test across combinations
    {
      const testCases = [
        {
          ...
          expected: 'durov-cap:all:all:TON',
        },
        {
          opts: {
          ...
          expected: 'pepe-hat:gold:space:STARS',
        },
        {
          opts: {
            collectionId: 'plush-bear',
            modelId: 'rare',
            backdropId: 'all',
            currency: 'TON' as const,
          },
          expected: 'plush-bear:rare:all:TON',
        },
      ];
      ...
      for (const tc of testCases) {
        ...
        assert.strictEqual(
          key,
          tc.expected,
          `Canonical key built correctly for ${tc.opts.collectionId}`
        );
      ...
      console.log(
        '✓ Steps 1-3 passed: Canonical instrumentKey generation & parameter parsing verified for TON and STARS'
      );
    }
    ...
    // 2. Comprehensive E2E flow across ALL 9 timeframes
    {
      for (const tf of timeframes) {
        ...
        assert.strictEqual(
          emptyHist.candles.length,
          0,
          `Empty history returns 0 candles for timeframe ${tf}`
        );
        ...
        assert.strictEqual(
          saleEvents1.length,
          3,
          '1 snapshot + 1 sale + 1 candle_update event emitted'
        );
        ...
        assert.strictEqual(
          msStart,
          activeC.startTime,
          'Millisecond <-> second round-trip lossfree'
        );
      ...
      console.log(
        '✓ Steps 4-18, 24 passed: Verified ingestion, OHLCV, Socket.io, floor price & duplicate handling across ALL 9 timeframes'
      );
    }
    ...
    // 3. Timeframe Switching & Stream Isolation Test (Steps 19-20)
    {
      ...
      assert.strictEqual(
        receivedTfEvents.includes('1m'),
        false,
        'Did NOT receive unsubscribed 1m event'
      );

      handleDisconnect(socket);
      console.log(
        '✓ Steps 19-20 passed: Timeframe switching strictly unsubscribes previous timeframe stream'
      );
    }
    ...
    // 4. Reconnect Recovery, Late Sale Correction & Revision Display (Steps 21-23)
    {
      ...
      console.log(
        '✓ Steps 21-23 passed: Reconnect state recovery, late sale correction & revision incrementing verified'
      );
    }
  });
});
