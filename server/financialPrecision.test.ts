...
describe('Stage 4: Financial Precision & OHLCV Scenarios', () => {
  test('Runs all 27 Financial Precision & OHLCV Scenarios', () => {
    ...
    // ----------------------------------------------------
    {
      ...
      assert(
        candle.quoteVolume === '0.3',
        `Test 3: QuoteVolume expected "0.3", got "${candle.quoteVolume}"`
      );
    ...
    // ----------------------------------------------------
    {
      ...
      assert(
        candle.quoteVolume === '0.3',
        `Test 4: QuoteVolume expected "0.3", got "${candle.quoteVolume}"`
      );
      ...
      console.log(
        '✓ Test 4 passed: Two sales 0.1 and 0.2 sum to quoteVolume=0.3 with correct high/low'
      );
    }
    ...
    // ----------------------------------------------------
    {
      ...
      const candle = res2.candles?.find((c) => c.timeframe === '1m')!;
      assert(
        candle.sumQuantity === '4',
        `Test 5: sumQuantity expected "4", got "${candle.sumQuantity}"`
      );
    ...
    // ----------------------------------------------------
    {
      ...
      const candle = res.candles?.find((c) => c.timeframe === '1m')!;
      assert(
        candle.quoteVolume === '1.000000000999999999',
        `Test 6: High precision quoteVolume expected "1.000000000999999999", got "${candle.quoteVolume}"`
      );
    ...
    // ----------------------------------------------------
    {
      ...
      const candle = res.candles?.find((c) => c.timeframe === '1m')!;
      assert(
        candle.quoteVolume === '2000000000000000',
        `Test 7: Large price quoteVolume expected "2000000000000000", got "${candle.quoteVolume}"`
      );
    ...
    // ----------------------------------------------------
    {
      ...
      const candle = res.candles?.find((c) => c.timeframe === '1m')!;
      assert(
        candle.volume === largeQty,
        `Test 8: Volume expected "${largeQty}", got "${candle.volume}"`
      );
      assert(
        candle.quoteVolume === '10000000000000000',
        `Test 8: QuoteVolume expected "10000000000000000", got "${candle.quoteVolume}"`
      );
    ...
    // ----------------------------------------------------
    {
      ...
      const candle = res2.candles?.find((c) => c.timeframe === '1m')!;
      assert(
        candle.high === '0.3000000001',
        `Test 9: High expected "0.3000000001", got "${candle.high}"`
      );
    ...
    // ----------------------------------------------------
    {
      ...
      const candle = res2.candles?.find((c) => c.timeframe === '1m')!;
      assert(
        candle.low === '0.2999999999',
        `Test 10: Low expected "0.2999999999", got "${candle.low}"`
      );
    ...
    // ----------------------------------------------------
    {
      ...
      assert(
        candle.open === '90',
        `Test 12: Tie-breaker open expected "90" (sale-a < sale-b), got "${candle.open}"`
      );
      assert(
        candle.close === '100',
        `Test 12: Tie-breaker close expected "100" (sale-b > sale-a), got "${candle.close}"`
      );
    ...
    // ----------------------------------------------------
    {
      ...
      const candle = res.candles?.find((c) => c.timeframe === '1m')!;
      assert(
        candle.quoteVolume === '200',
        `Test 18: QuoteVolume expected "200", got "${candle.quoteVolume}"`
      );
    ...
    // ----------------------------------------------------
    {
      ...
      assert(
        ...
        'Test 24: Updated exactly 9 timeframes'
      );
      const timeframes = res.candles?.map((c) => c.timeframe).sort();
      const expectedTfs = ['1s', '1m', '5m', '15m', '1h', '4h', '1d', '1w', '1M'].sort();
      assert(
        JSON.stringify(timeframes) === JSON.stringify(expectedTfs),
        'Test 24: All 9 valid timeframes present'
      );
    ...
    // ----------------------------------------------------
    {
      ...
      assert(
        candleAfterDup.revision === 2,
        `Test 27: Revision remains 2 after duplicate, got ${candleAfterDup.revision}`
      );
