  ...
  test('Runs Stage 14 Performance Scenarios', async () => {
    ...
    // Scenario 1: Large volume of candles memory bounding in CandleStore
    {
      ...
      assert.strictEqual(
        sorted[0].startTime,
        baseTime + 1000 * 60000,
        'Oldest 1000 candles pruned correctly'
      );
    ...
    // Scenario 2: Large volume of sales memory bounding on Backend
    {
      ...
      assert.ok(
        allSales.length <= 20000,
        `allSales bounded to 20,000 recent sales max, actual: ${allSales.length}`
      );
      assert.strictEqual(
        processedSaleIds.size,
        25000,
        'processedSaleIds tracks recent dedupe keys without leak'
      );
      ...
      console.log(
        '✓ Scenario 2 passed: Large volume of sales bounded on backend without breaking deduplication'
      );
    }
    ...
    // Scenario 3: Closed candles bounding per timeframe
    {
      ...
      assert.ok(
        closed.length <= 5000,
        `closedCandles for ${tf1m} bounded to 5000 max, actual: ${closed.length}`
      );
    ...
    // Scenario 4: Multiple clients & socket disconnect cleanup
    {
      ...
      console.log(
        '✓ Scenario 4 passed: 100 concurrent client sockets managed and disconnected cleanly without memory leak'
      );
    }
    ...
    // Scenario 5: Duplicate subscription handling
    {
      ...
      console.log(
        '✓ Scenario 5 passed: Duplicate subscription handled without creating redundant streams or leaks'
      );
    }
    ...
    // Scenario 7: REST history limit enforcement & pagination
    {
      ...
      const page2 = getMarketCandlesHistory(instA, tf1m, {
        limit: 1000,
        cursor: page1.nextCursor!,
      });
      assert.ok(page2.candles.length > 0, 'Page 2 returns remaining candles');
      assert.strictEqual(
        page2.candles[0].startTime > Number(page1.nextCursor),
        true,
        'Pagination strictly strictly advances after cursor'
      );

      console.log(
        '✓ Scenario 7 passed: REST history limit enforced and cursor pagination verified'
      );
    }
    ...
    // Scenario 8: Incremental candle update performance
    {
      ...
      assert.ok(
        duration < 1500,
        `1,000 incremental sale processing executed in ${duration.toFixed(2)}ms (<1500ms)`
      );

      console.log(
        `✓ Scenario 8 passed: 1,000 incremental sale updates executed in ${duration.toFixed(2)}ms`
      );
    }
