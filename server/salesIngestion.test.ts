...
describe('Stage 3: Complete 25 Sales Ingestion, Validation & Deduplication Scenarios', () => {
  test('Runs Stage 3 Sales Ingestion Scenarios', () => {
    ...
    console.log(
      '=== Running Stage 3: Complete 25 Sales Ingestion, Validation & Deduplication Scenarios ==='
    );
    ...
    // ----------------------------------------------------
    {
      ...
      assert(
        res.candles !== undefined && res.candles.length === 9,
        'Test 1: Must update 9 timeframes'
      );
    ...
    // ----------------------------------------------------
    {
      ...
      assert(
        acceptCompletedSale(cancelled).reason === 'not_completed',
        'Test 5: cancelled rejected'
      );
      assert(acceptCompletedSale(reverted).reason === 'not_completed', 'Test 6: reverted rejected');
      assert(
        acceptCompletedSale(unknown).reason === 'not_completed',
    ...
    // ----------------------------------------------------
    {
      ...
      assert(
        acceptCompletedSale(noIdNoFallback).accepted === false,
        'Test 8a: No ID and no fallback rejected'
      );
      ...
      assert(
        fallbackRes.dedupeKey === '0xabc_gift-99_1710000000000',
        'Test 8b: Correct composite dedupe key'
      );

      console.log(
        '✓ Test 8 passed: Missing ID without fallback rejected; composite fallback accepted'
      );
    }
    ...
    // ----------------------------------------------------
    {
      ...
      assert(
        acceptCompletedSale(saleFuture).accepted === true,
        'Test 10: Future sale accepted first'
      );
      assert(
        acceptCompletedSale(salePast).accepted === true,
        'Test 11: Past/late sale accepted second'
      );
    ...
    // ----------------------------------------------------
    {
      ...
      assert(
        getHistory('c1:m1:b1:TON', '1m', 0, 2000000000000).length === 1,
        'Isolation c1:m1:b1:TON'
      );
      assert(
        getHistory('c2:m1:b1:TON', '1m', 0, 2000000000000).length === 1,
        'Isolation c2:m1:b1:TON'
      );
      assert(
        getHistory('c1:m2:b1:TON', '1m', 0, 2000000000000).length === 1,
        'Isolation c1:m2:b1:TON'
      );
      assert(
        getHistory('c1:m1:b2:TON', '1m', 0, 2000000000000).length === 1,
        'Isolation c1:m1:b2:TON'
      );
      assert(
        getHistory('c1:m1:b1:STARS', '1m', 0, 2000000000000)[0].close === '500',
        'Isolation STARS price'
      );

      console.log(
        '✓ Test 12, 13, 14, 15 passed: Strict isolation across collection, model, backdrop, currency'
      );
    }
    ...
    // ----------------------------------------------------
    {
      ...
      assert(
        acceptCompletedSale(negPrice).reason === 'invalid',
        'Test 17: Negative price rejected'
      );
    ...
    // ----------------------------------------------------
    {
      ...
      assert(
        acceptCompletedSale(negQty).reason === 'invalid',
        'Test 19: Negative quantity rejected'
      );
    ...
    // ----------------------------------------------------
    {
      ...
      assert(
        acceptCompletedSale(infPrice).reason === 'invalid',
        'Test 20: Infinity price rejected'
      );
    ...
    // ----------------------------------------------------
    {
      ...
      console.log(
        '✓ Test 21 & 22 passed: Single market change on accepted sale, no changes on redelivery'
      );
    }
    ...
    // ----------------------------------------------------
    {
      ...
      assert(
        acceptCompletedSale(secondsSale).reason === 'invalid',
        'Test 24: Timestamp in seconds rejected'
      );
