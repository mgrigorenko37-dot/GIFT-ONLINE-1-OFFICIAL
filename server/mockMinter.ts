...
export function simulateSales(io: any) {
  ...
  const isSimulationEnabled = process.env.SIMULATION_MODE === "true" || process.env.ENABLE_SIMULATION === "true";

  if (!isSimulationEnabled) {
    console.log("[mockMinter] Simulation disabled. (Set SIMULATION_MODE=true or ENABLE_SIMULATION=true to enable)");
    return;
  }

  if (isProduction && process.env.ALLOW_SIMULATION_IN_PRODUCTION !== "true") {
    console.warn("[mockMinter] SAFETY REJECTION: Mock simulation is disabled in production environments by default.");
    return;
  }
