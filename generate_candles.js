...
async function generateCandlesForAsset(client, asset) {
  ...
  for (const tf of TIMEFRAMES) {
    let numCandles = 0;
    if (tf === '1s') numCandles = 100; // drastically reduce to avoid OOM or timeout on 851 assets
    ...
    for (let i = 0; i < numCandles; i++) {
      ...
      values.push(`($${paramCounter++}, $${paramCounter++}, $${paramCounter++}, $${paramCounter++}, $${paramCounter++}, $${paramCounter++}, $${paramCounter++}, $${paramCounter++}, $${paramCounter++}, 0, 0, 0, 0, 0, '', '', true, 1, $${paramCounter++})`);
...
async function run() {
  ...
  try {
    const { rows: assets } = await client.query("SELECT id, current_price_gx FROM gift_variants WHERE id LIKE '%_CLASSIC%' OR id LIKE '%_GOLD%' OR id LIKE '%_NEON%' OR id LIKE '%_CYBER%' OR id LIKE '%_DIAMOND%'");
