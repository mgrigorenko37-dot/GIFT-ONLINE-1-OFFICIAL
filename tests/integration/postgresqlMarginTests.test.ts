...
describe('Postgres Margin and Isolation Tests', () => {
  ...
  async function getBalance(userId: string, currency: string) {
    const res = await pool.query('SELECT * FROM te_balances WHERE user_id = $1 AND currency = $2', [
      userId,
      currency,
    ]);
  ...
  async function getPosition(userId: string, instrumentKey: string) {
    const res = await pool.query(
      'SELECT * FROM te_positions WHERE user_id = $1 AND instrument_key = $2',
  ...
  it('32. Liquidation idempotency - repeated call with same executionId does not duplicate trades/fees/balances/outbox', async () => {
    ...
    await pool.query(
      'UPDATE te_positions SET mark_price = 30 WHERE user_id = $1 AND instrument_key = $2',
      [userId, 'TON-USD']
    );
    ...
    const tradesCount1 = (
      await pool.query('SELECT COUNT(*) FROM te_trades WHERE user_id = $1', [userId])
    ).rows[0].count;
    const execCount1 = (
      await pool.query('SELECT COUNT(*) FROM te_executions WHERE user_id = $1', [userId])
    ).rows[0].count;
    const outboxCount1 = (
      await pool.query('SELECT COUNT(*) FROM te_outbox_events WHERE user_id = $1', [userId])
    ).rows[0].count;
    ...
    console.log(
      'PASS RUNTIME: 32. Liquidation idempotency - repeated call with same executionId does not duplicate trades/fees/balances/outbox'
    );
  });
  ...
  it('33. MARGIN_CALL - sets MarginCall on approach, creates outbox event, keeps position open, avoids duplicate event spam', async () => {
    ...
    if (!order || order.status === 'Rejected')
      throw new Error('Order rejected: ' + order?.rejectionReason);
    ...
    const mcEvents1 = (
      await pool.query(
        "SELECT COUNT(*) FROM te_outbox_events WHERE user_id = $1 AND event_type = 'marginCall'",
        [userId]
      )
    ).rows[0].count;
    ...
    console.log(
      'PASS RUNTIME: 33. MARGIN_CALL - sets MarginCall on approach, creates outbox event, keeps position open, avoids duplicate event spam'
    );
  });
  ...
  it('34. OUTBOX - Liquidation emits trade, order, position, balance, and ledger events with required fields', async () => {
    ...
    console.log(
      'PASS RUNTIME: 34. OUTBOX - Liquidation emits trade, order, position, balance, and ledger events with required fields'
    );
  });
  ...
  it('35. ROLLBACK - Outbox events are not persisted if liquidation transaction rolls back', async () => {
    ...
    const outboxEvents = (
      await pool.query(
        "SELECT * FROM te_outbox_events WHERE user_id = $1 AND event_type IN ('tradeExecuted', 'orderUpdated', 'positionUpdated', 'balanceUpdated', 'ledgerUpdated')",
        [userId]
      )
    ).rows;
    ...
    console.log(
      'PASS RUNTIME: 35. ROLLBACK - Outbox events are not persisted if liquidation transaction rolls back'
    );
  });
  ...
  it('36. FORMULAS - Explicit verification of accounting formulas (initialMargin, usedMargin, maintenanceMargin, Long/Short unrealizedPnl, equity, threshold, fee, realizedPnl)', async () => {
    ...
    await pool.query(
      'UPDATE te_positions SET mark_price = 8 WHERE user_id = $1 AND instrument_key = $2',
      [userId, 'TON-USD']
    );
    ...
    await pool.query(
      'UPDATE te_positions SET mark_price = 12 WHERE user_id = $1 AND instrument_key = $2',
      [userIdShort, 'TON-USD']
    );
    ...
    await pool.query(
      'UPDATE te_balances SET available_balance = 10 WHERE user_id = $1 AND currency = $2',
      [userId, 'TON']
    );
    await pool.query(
      'UPDATE te_positions SET mark_price = 0 WHERE user_id = $1 AND instrument_key = $2',
      [userId, 'TON-USD']
    );
  ...
  it('37. LONG LIQUIDATION - Deterministic test (qty=1, avgEntryPrice=10, markPrice=4)', async () => {
    ...
    // Set markPrice = 4
    await pool.query(
      'UPDATE te_positions SET mark_price = 4 WHERE user_id = $1 AND instrument_key = $2',
      [userId, 'TON-USD']
    );
    ...
    await pool.query(
      'UPDATE te_balances SET available_balance = 6.175 WHERE user_id = $1 AND currency = $2',
      [userId, 'TON']
    );

    // Execute Liquidation
    const liqRes = await engine.liquidateUser(null, userId, 'TON', {
      executionId: 'exec_det_long_1',
      tradeId: 't_det_long_liq_exec',
    });
    expect(liqRes).not.toBeNull();
  ...
  it('38. SHORT LIQUIDATION - Deterministic test (qty=1, avgEntryPrice=4, markPrice=10)', async () => {
    ...
    // Set markPrice = 10
    await pool.query(
      'UPDATE te_positions SET mark_price = 10 WHERE user_id = $1 AND instrument_key = $2',
      [userId, 'TON-USD']
    );
    ...
    await pool.query(
      'UPDATE te_balances SET available_balance = 6.19 WHERE user_id = $1 AND currency = $2',
      [userId, 'TON']
    );

    // Execute Liquidation
    const liqRes = await engine.liquidateUser(null, userId, 'TON', {
      executionId: 'exec_det_short_1',
      tradeId: 't_det_short_liq_exec',
    });
    expect(liqRes).not.toBeNull();
    ...
    expect(Number(liqTrade.fee)).toBeCloseTo(0.1);
    ...
    const balAfter = await getBalance(userId, 'TON');
    expect(balAfter.locked).toBe(0);
    expect(balAfter.available).toBeCloseTo(0.09);
    expect(balAfter.pnl).toBeCloseTo(-6);
  ...
  it('39. FEE MODEL B - Explicit verification of fee, liquidation_fee, total_fees, available_balance, idempotency, and currency isolation', async () => {
    ...
    await pool.query(
      ...
      [userId, 'TON', 100.0, 0, 0, 0, nowMs, nowMs]
    );
    await pool.query(
      'INSERT INTO te_balances (user_id, currency, available_balance, locked_balance, total_fees, realized_pnl, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)',
      [userId, 'STARS', 500.0, 0, 0, 0, nowMs, nowMs]
    );
    ...
    expect(balAfterTrade1.locked).toBeCloseTo(16.0);
    ...
    expect(starsBalAfterTrade1.available).toBe(500.0);
    ...
    expect(starsBalAfterLiq.available).toBe(500.0);

    // --- STEP 3: Idempotency - Repeated Liquidation Call ---
    const liqResRepeat = await engine.liquidateUser(null, userId, 'TON', {
      executionId: liqExecId,
    });
    ...
    expect(starsTradeFee).toBeCloseTo(0.5);
    ...
    expect(starsBalFinal.fees).toBeCloseTo(0.5);
    expect(starsBalFinal.available).toBeCloseTo(499.5); // 500.00 - 0.50 fee (with 200.00 locked margin)
  ...
  it('40. QTY SEMANTICS & VALIDATION - Verification of Position.qty=0 after liquidation, Trade.qty=closedQty, Execution.fillQty=closedQty, Order.executedQty=closedQty, and qty boundary constraints', async () => {
    ...
    const posBeforeLiqRes = await pool.query(
      'SELECT * FROM te_positions WHERE user_id = $1 AND instrument_key = $2',
      [userId, 'TON-USD']
    );
    ...
    await pool.query(
      'UPDATE te_positions SET mark_price = 4, unrealized_pnl = -30 WHERE user_id = $1 AND instrument_key = $2',
      [userId, 'TON-USD']
    );
    // Set available balance = 20 (equity = 20 - 30 = -10 <= maintenance margin 2.0)
    await pool.query(
      'UPDATE te_balances SET available_balance = 20 WHERE user_id = $1 AND currency = $2',
      [userId, 'TON']
    );

    const liqExecId = 'exec_qty_liq_' + Date.now();
    const liqTradeId = 't_qty_liq_' + Date.now();
    const liqTrade = await engine.liquidateUser(null, userId, 'TON', {
      executionId: liqExecId,
      tradeId: liqTradeId,
    });
    ...
    const posAfterLiqRes = await pool.query(
      'SELECT * FROM te_positions WHERE user_id = $1 AND instrument_key = $2',
      [userId, 'TON-USD']
    );
    ...
    const execRes = await pool.query('SELECT * FROM te_executions WHERE execution_id = $1', [
      liqExecId,
    ]);
    ...
    await expect(engine.executeTrade(openOrdId, 0, 10.0)).rejects.toThrow();
    ...
    await pool.query(
      'UPDATE te_balances SET available_balance = 100 WHERE user_id = $1 AND currency = $2',
      [userId, 'TON']
    );
    ...
    const pos2AfterClose = await pool.query(
      'SELECT * FROM te_positions WHERE user_id = $1 AND instrument_key = $2',
      [userId, 'TON-USD']
    );
    ...
    const allPositions = await pool.query('SELECT * FROM te_positions WHERE user_id = $1', [
      userId,
    ]);
    ...
    const allExecutions = await pool.query(
      'SELECT * FROM te_executions WHERE order_id IN (SELECT order_id FROM te_orders WHERE user_id = $1)',
      [userId]
    );
    const allOutboxEvents = await pool.query('SELECT * FROM te_outbox_events WHERE user_id = $1', [
      userId,
    ]);
    ...
    expect(Number(liqTradeDbRow.liquidation_fee ?? liqTradeDbRow.fee ?? 0)).toBeGreaterThanOrEqual(
      0
    );
    ...
    console.log(
      `- Execution.fillQty:               ${execRes.rows[0].fill_qty} (closed position size = 5)`
    );
    console.log(
      `- Order.executedQty:               ${orderRes.rows[0].executed_qty} (closed position size = 5)`
    );
    console.log(
      `- Trade.qty > pos.qty constraint:  Capped at ${closeTradeExceed?.qty} (initial position size = 2)`
    );
  ...
  it('41. Repeated Liquidation - Idempotency and Non-duplication Verification', async () => {
    ...
    let pos0 = (
      await pool.query('SELECT * FROM te_positions WHERE user_id = $1 AND instrument_key = $2', [
        userId,
        'TON-USD',
      ])
    ).rows[0];
    ...
    await pool.query(
      'UPDATE te_positions SET mark_price = 1.0, unrealized_pnl = -18.0 WHERE user_id = $1 AND instrument_key = $2',
      [userId, 'TON-USD']
    );
    await pool.query(
      'UPDATE te_balances SET available_balance = 18.05 WHERE user_id = $1 AND currency = $2',
      [userId, currency]
    );
    ...
    const bal1 = (
      await pool.query('SELECT * FROM te_balances WHERE user_id = $1 AND currency = $2', [
        userId,
        currency,
      ])
    ).rows[0];
    ...
    const execs1 = (await pool.query('SELECT * FROM te_executions WHERE user_id = $1', [userId]))
      .rows;
    const outbox1 = (
      await pool.query('SELECT * FROM te_outbox_events WHERE user_id = $1', [userId])
    ).rows;
    ...
    const execs2 = (await pool.query('SELECT * FROM te_executions WHERE user_id = $1', [userId]))
      .rows;
    const outbox2 = (
      await pool.query('SELECT * FROM te_outbox_events WHERE user_id = $1', [userId])
    ).rows;
    ...
    console.log(
      'PASS RUNTIME: 41. Repeated Liquidation - Idempotency and Non-duplication Verification'
    );
  });
  ...
  it('42. Parallel Liquidation - Concurrent Calls Verification', async () => {
    ...
    const execs = (await pool.query('SELECT * FROM te_executions WHERE user_id = $1', [userId]))
      .rows;
    const bal = (
      await pool.query('SELECT * FROM te_balances WHERE user_id = $1 AND currency = $2', [
        userId,
    ...
    const liqTrades = trades.filter(
      (t) => t.trade_id.startsWith('t_liq_') || t.order_id.startsWith('ord_liq_')
    );
    ...
    console.log(
      'PARALLEL CONCURRENCY VERIFIED (Single-process DB transactions): 42. Parallel Liquidation passed'
    );
  });
  ...
  it('43. SIMULATION FUTURES FUNDING - Long & Short funding calculations and balance updates', async () => {
    ...
    expect(longPayment!.fundingAmount).toBe(0.5);
    ...
    expect(shortPayment!.fundingAmount).toBe(-0.5);
    ...
    expect(balLongAfter).toBeCloseTo(balLongBefore - 0.5, 4);
    expect(balShortAfter).toBeCloseTo(balShortBefore + 0.5, 4);
    ...
    console.log(
      'SIMULATION FUTURES FUNDING VERIFIED: 43. Long & Short funding calculations passed'
    );
  });
  ...
  it('44. FUNDING IDEMPOTENCY & NON-DUPLICATION - Repeated funding with same timestamp is ignored', async () => {
    ...
    const dbPayments = (
      await pool.query('SELECT * FROM te_funding_payments WHERE user_id = $1', [userId])
    ).rows;
  ...
  it('45. FUNDING SAFETY & ISOLATION - Funding does not touch position qty, status, or trigger reverse/liquidation', async () => {
    ...
    const posBefore = (await pool.query('SELECT * FROM te_positions WHERE user_id = $1', [userId]))
      .rows[0];
    ...
    const posAfter = (await pool.query('SELECT * FROM te_positions WHERE user_id = $1', [userId]))
      .rows[0];
  ...
  it('46. FUNDING ELIGIBILITY CONSTRAINTS - Only active OPEN positions with qty>0 at funding timestamp receive funding', async () => {
    ...
    expect(partialPayment!.fundingAmount).toBe(0.3);

    console.log(
      'FUNDING ELIGIBILITY VERIFIED: 46. Closed, new, and zero-qty positions correctly excluded'
    );
  });
  ...
  it('47. FUNDING CURRENCY ISOLATION & ROLLBACK SAFETY - TON and STARS funding strictly isolated', async () => {
    ...
    expect(tonPayments[0].fundingAmount).toBe(0.5); // 10 * 5 * 0.01 = 0.50 TON
    ...
    expect(balTonAfterTon).toBeCloseTo(99.5, 4);
    ...
    expect(balTonAfterStars).toBeCloseTo(99.5, 4);
    expect(balStarsAfterStars).toBeCloseTo(502.0, 4);

    // Verify database record currency and outbox event currency
    const dbPayments = (
      await pool.query('SELECT * FROM te_funding_payments WHERE user_id = $1 ORDER BY currency', [
        userId,
      ])
    ).rows;
    expect(dbPayments.length).toBe(2);
    ...
    const outboxEvents = (
      await pool.query(
        "SELECT * FROM te_outbox_events WHERE user_id = $1 AND event_type = 'fundingProcessed' ORDER BY currency",
        [userId]
      )
    ).rows;
    ...
    console.log(
      'FUNDING CURRENCY ISOLATION & ROLLBACK VERIFIED: 47. TON & STARS isolated, rollback verified'
    );
  });
  ...
  it('48. FUNDING PARAMETER CONFLICT & STRICT IDEMPOTENCY - Conflict on mismatched parameters, idempotency on matching parameters', async () => {
    ...
    const paymentsCount1 = (
      await pool.query('SELECT COUNT(*) FROM te_funding_payments WHERE user_id = $1', [userId])
    ).rows[0].count;
    const outboxCount1 = (
      await pool.query('SELECT COUNT(*) FROM te_outbox_events WHERE user_id = $1', [userId])
    ).rows[0].count;
    ...
    console.log(
      'FUNDING STRICT IDEMPOTENCY & CONFLICT VERIFIED: 48. Matching parameters return existing result; conflicting parameters throw error and preserve state'
    );
  });
  ...
  it('49. POSITIVE & NEGATIVE FUNDING RATE MECHANICS - Long pays/receives, Short pays/receives, rate=0, large decimal, min allowed rate', async () => {
    ...
    const outboxLong1 = await pool.query(
      `SELECT * FROM te_outbox_events WHERE user_id = $1  ORDER BY id DESC LIMIT 20`,
      [userLong]
    );
    const ledgerLong1 = outboxLong1.rows.find(
      (r) => r.event_type === 'ledgerUpdated' && JSON.parse(r.payload).fundingTimestamp === ts1
    );
    ...
    const balanceLong1 = outboxLong1.rows.find(
      (r) => r.event_type === 'balanceUpdated' && JSON.parse(r.payload).fundingTimestamp === ts1
    );
    ...
    expect(balanceLongPayload1.previousBalance - balanceLongPayload1.availableBalance).toBeCloseTo(
      res1Long[0].fundingAmount,
      4
    );
    ...
    console.log(
      'POSITIVE & NEGATIVE FUNDING MECHANICS VERIFIED: 49. Long/Short pay/receive, zero, high decimal, and min rate all passed'
    );
  });
  ...
  it('50. FUNDING RATE VALIDATION & NEGATIVE BALANCE SAFETY - Rejects NaN, Infinity, out-of-range rates, and prevents negative balance', async () => {
    ...
    await pool.query(
      'UPDATE te_balances SET available_balance = 0.25 WHERE user_id = $1 AND currency = $2',
      [poorUser, currency]
    );
    ...
    console.log(
      'FUNDING VALIDATION & BALANCE SAFETY VERIFIED: 50. NaN, Infinity, out-of-bounds rejected; negative balance prevented'
    );
  });
  ...
  it('51. FUNDING & MARGIN INTEGRATION - Verifies available balance change, locked balance invariant, used margin invariant, equity recalculation, margin ratio deterioration, margin call transition, position invariant (avgEntryPrice, qty, side)', async () => {
    ...
    const balPayerBefore = await pool.query(
    ...
    const balRecBefore = await pool.query(
      'SELECT available_balance, locked_balance FROM te_balances WHERE user_id = $1 AND currency = $2',
    ...
    const posPayerBefore = (await engine.getAllPositions(userPayer)).find(
      (p) => p.instrumentKey === 'TON'
    );
    ...
    const balPayerAfter = await pool.query(
      'SELECT available_balance, locked_balance FROM te_balances WHERE user_id = $1 AND currency = $2',
      [userPayer, currency]
    );
    ...
    const balRecAfter = await pool.query(
      'SELECT available_balance, locked_balance FROM te_balances WHERE user_id = $1 AND currency = $2',
      [userReceiver, currency]
    );
    ...
    const posPayerAfter = (await engine.getAllPositions(userPayer)).find(
      (p) => p.instrumentKey === 'TON'
    );
    ...
    await pool.query(
      'UPDATE te_balances SET available_balance = 5.0 WHERE user_id = $1 AND currency = $2',
      [userPayer, currency]
    ); // usedMargin=100, maintenanceMargin=5.0
    ...
    console.log(
      'FUNDING & MARGIN INTEGRATION VERIFIED: 51. All 11 funding & margin invariants confirmed'
    );
  });
  ...
  it('52. FUNDING AFTER RESTART (VARIANT A) - Worker catches up missed funding periods sequentially without duplicates or invalid positions', async () => {
    ...
    console.log(
      'FUNDING AFTER RESTART (VARIANT A) VERIFIED: 52. Missed periods processed sequentially, no duplicates, closed positions excluded'
    );
  });
  ...
  it('53. FUNDING WORKER LIFECYCLE & CONCURRENCY - Explicit start/stop, no uncontrolled import timers, idempotency & PostgreSQL row locking', async () => {
    ...
    console.log(
      'FUNDING WORKER LIFECYCLE VERIFIED: 53. Explicit start/stop, duplicate start prevented, PostgreSQL row-locking & idempotency confirmed'
    );
  });
  ...
  it('54. FUNDING OUTBOX EVENTS - Emits fundingUpdated, balanceUpdated, ledgerUpdated, positionUpdated with required fields after COMMIT', async () => {
    ...
    const outboxRows = (
      await pool.query(
        'SELECT * FROM te_outbox_events WHERE user_id = $1 ORDER BY created_at ASC',
        [obUser]
      )
    ).rows;
    const eventTypes = outboxRows.map((r) => r.event_type);
    ...
    for (const eventType of targetTypes) {
      ...
      const payload =
        typeof eventRow.payload === 'string' ? JSON.parse(eventRow.payload) : eventRow.payload;
    ...
    console.log(
      'FUNDING OUTBOX EVENTS VERIFIED: 54. All outbox events emitted with required fields after transaction COMMIT'
    );
  });
  ...
  it('55. POSITION-LEVEL FUNDING UNIQUENESS - Multi-position funding for same user & timestamp succeed separately, repeat for same position returns duplicate', async () => {
    ...
    const dbRows = (
      await pool.query(
        'SELECT * FROM te_funding_payments WHERE user_id = $1 AND funding_timestamp = $2',
        [multiUser, fTs]
      )
    ).rows;
    ...
    console.log(
      'POSITION-LEVEL FUNDING UNIQUENESS VERIFIED: 55. Separate funding payments for multiple positions of same user & timestamp; repeat position rejected/deduplicated'
    );
  });
  ...
  it('56. STRICT CURRENCY VALIDATION & NO DEFAULT FALLBACK - Rejects position without currency, unknown currency, TON/STARS mismatch, verifies correct TON/STARS and complete rollback on error', async () => {
    ...
    const getSnapshot = async () => {
      const balances = (
        await pool.query(
          'SELECT currency, available_balance, locked_balance FROM te_balances WHERE user_id = $1 ORDER BY currency',
          [testUser]
        )
      ).rows;
      const payments = (
        await pool.query('SELECT * FROM te_funding_payments WHERE user_id = $1', [testUser])
      ).rows;
      const outbox = (
        await pool.query('SELECT * FROM te_outbox_events WHERE user_id = $1', [testUser])
      ).rows;
    ...
    const tonPosRow = (
      await pool.query(
        `SELECT position_id FROM te_positions WHERE user_id = $1 AND instrument_key = 'TON'`,
        [testUser]
      )
    ).rows[0];
    ...
    const starsPosRow = (
      await pool.query(
        `SELECT position_id FROM te_positions WHERE user_id = $1 AND instrument_key = 'STARS'`,
        [testUser]
      )
    ).rows[0];
    ...
    console.log(
      'STRICT CURRENCY VALIDATION VERIFIED: 56. Rejects missing/unknown currency and currency mismatch; verifies correct TON & STARS funding and clean rollback on error'
    );
  });
  ...
  it('57. PER-POSITION CATCH-UP CURSOR ISOLATION - Independent cursor tracking per position/instrument/currency/interval', async () => {
    ...
    const paymentsABefore = (
      await pool.query(
        'SELECT funding_timestamp FROM te_funding_payments WHERE position_id = $1 ORDER BY funding_timestamp',
        [posAId]
      )
    ).rows;
    ...
    console.log(
      'PER-POSITION CATCH-UP CURSOR ISOLATION VERIFIED: 57. Position A processed 2, 3, 4; Position B processed only 4; cursors isolated'
    );
  });
  ...
  it('58. HISTORICAL MARK PRICE FOR FUNDING CATCH-UP - Uses historical snapshots instead of current markPrice and controlled SKIPPED on missing snapshot', async () => {
    ...
    const dbPayments = (
      await pool.query(
        'SELECT funding_timestamp, mark_price, funding_rate, funding_amount FROM te_funding_payments WHERE user_id = $1 ORDER BY funding_timestamp',
        [histUser]
      )
    ).rows;
    ...
    console.log(
      'HISTORICAL MARK PRICE FOR FUNDING CATCH-UP VERIFIED: 58. Uses historical snapshots (10 and 20) instead of current markPrice (100); missing snapshot explicitly marked as SKIPPED'
    );
  });
  ...
  it('59. HISTORICAL QTY FOR FUNDING CATCH-UP - Uses historical position qty instead of current qty and skips if missing', async () => {
    ...
    await pool.query(
      'DELETE FROM te_position_snapshots WHERE position_id = $1 AND valid_from > $2',
      [posId, t2 + 1000]
    );
    ...
    console.log(
      'HISTORICAL QTY FOR FUNDING CATCH-UP VERIFIED: 59. Uses qty=10 for t1, qty=5 for t2, explicitly skips t3 due to missing snapshot'
    );
  });
  ...
  it('60. FUNDING INSUFFICIENT BALANCE - No silent clamping or partial deductions', async () => {
    ...
    await pool.query(
      'UPDATE te_balances SET available_balance = 2 WHERE user_id = $1 AND currency = $2',
      [user, currency]
    );
    const ts4 = ts3 + 1000;
    await engine.applyFundingRate(null, {
    ...
    await pool.query(
      'UPDATE te_balances SET available_balance = 1 WHERE user_id = $1 AND currency = $2',
      [userStars, currencyStars]
    );
    ...
    let outboxStars = (
      await pool.query(
        `SELECT * FROM te_outbox_events WHERE user_id = $1 AND event_type = 'marginCall'`,
        [userStars]
      )
    ).rows;
    ...
    console.log(
      'FUNDING INSUFFICIENT BALANCE VERIFIED: 60. No silent clamping or partial sum deduction.'
    );
  });
});
