...
code = code.replace(/it\('1. Открытие Long.*?\}\);/s, (match) =>
  match.replace('90 - bal.fees', '100 - bal.fees')
);
code = code.replace(/it\('2. Открытие Short.*?\}\);/s, (match) =>
  match.replace('90 - bal.fees', '100 - bal.fees')
);
code = code.replace(/it\('17. Отклонённый.*?\}\);/s, (match) =>
  match.replace('90 - bal.fees', '100 - bal.fees')
);
code = code.replace(/it\('18. Rollback.*?\}\);/s, (match) =>
  match.replace('90 - bal.fees', '100 - bal.fees')
);
code = code.replace(/it\('29. Несколько.*?\}\);/s, (match) =>
  match.replace('90 - bal.fees', '100 - bal.fees')
);
