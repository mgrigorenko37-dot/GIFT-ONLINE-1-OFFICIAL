...
async function main() {
  ...
  await pool.query(
    ...
    [user, 'TON', 1000, 0, Date.now(), Date.now()]
  );
  await pool.query(
    'INSERT INTO te_balances (user_id, currency, available_balance, locked_balance, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6)',
    [user, 'STARS', 1000, 0, Date.now(), Date.now()]
  );
  ...
  const posRes = await pool.query(
    'SELECT position_id, instrument_key FROM te_positions WHERE user_id = $1',
    [user]
  );
  let tonPos, starsPos;
  ...
  await pool.query('UPDATE te_position_snapshots SET valid_from = $1 WHERE position_id = $2', [
    t0,
    tonPos.position_id,
  ]);
  await pool.query('UPDATE te_position_snapshots SET valid_from = $1 WHERE position_id = $2', [
    t0,
    starsPos.position_id,
  ]);
  ...
  const runWorker = (name: string) => {
    return new Promise((resolve) => {
      const proc = spawn(
        'npx',
        ['tsx', 'multi_worker_7.ts', name, String(t4), String(intervalMs)],
        { env: process.env }
      );
  ...
  const payTon = await pool.query(
    'SELECT funding_timestamp FROM te_funding_payments WHERE position_id = $1 ORDER BY funding_timestamp ASC',
    [tonPos.position_id]
  );
  console.log(
    `TON Payments: ${payTon.rows.map((r) => (r.funding_timestamp === String(t1) ? 't1' : r.funding_timestamp === String(t2) ? 't2' : r.funding_timestamp === String(t3) ? 't3' : r.funding_timestamp === String(t4) ? 't4' : r.funding_timestamp)).join(', ')}`
  );
  ...
  console.log(
    `STARS Payments: ${payStars.rows.map((r) => (r.funding_timestamp === String(t1) ? 't1' : r.funding_timestamp === String(t2) ? 't2' : r.funding_timestamp === String(t3) ? 't3' : r.funding_timestamp === String(t4) ? 't4' : r.funding_timestamp)).join(', ')}`
  );

  const tonBal = await pool.query(
