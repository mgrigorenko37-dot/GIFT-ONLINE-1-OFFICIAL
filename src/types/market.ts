...
export function buildInstrumentKey(options: BuildInstrumentKeyOptions): string {
  ...
  const normModel =
    !modelId || typeof modelId !== 'string' || modelId.trim() === '' || modelId.trim() === 'any'
      ? DEFAULT_MODEL_ID
      : modelId.trim();

  const normBackdrop =
    !backdropId ||
    typeof backdropId !== 'string' ||
    backdropId.trim() === '' ||
    backdropId.trim() === 'any'
      ? DEFAULT_BACKDROP_ID
      : backdropId.trim();
...
export function parseInstrumentKey(instrumentKey: string): ParsedInstrumentKey {
  ...
  if (parts.length !== 4) {
    throw new Error(
      `Invalid instrumentKey format: "${instrumentKey}". Expected format "collectionId:modelId:backdropId:currency"`
    );
  }
  ...
  if (rawCurrency !== 'TON' && rawCurrency !== 'STARS') {
    throw new Error(
      `Invalid currency in instrumentKey: "${rawCurrency}". Expected "TON" or "STARS"`
    );
  }

  const modelId =
    !rawModel || rawModel.trim() === '' || rawModel.trim() === 'any' || rawModel.trim() === 'all'
      ? DEFAULT_MODEL_ID
      : rawModel.trim();
  ...
  const backdropId =
    !rawBackdrop ||
    rawBackdrop.trim() === '' ||
    rawBackdrop.trim() === 'any' ||
    rawBackdrop.trim() === 'all'
      ? DEFAULT_BACKDROP_ID
      : rawBackdrop.trim();
