...
async function main() {
  ...
  const posRes = await pool.query('SELECT position_id FROM te_positions WHERE user_id = $1', [
    user,
  ]);
  ...
  let outbox = await pool.query(
    `SELECT COUNT(*) as c FROM te_outbox_events WHERE user_id = $1 AND event_type = 'ledgerUpdated' AND payload LIKE '%FUNDING%'`,
    [user]
  );
