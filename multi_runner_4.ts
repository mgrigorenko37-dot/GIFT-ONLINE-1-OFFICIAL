...
async function main() {
  ...
  const posRes = await pool.query(
    'SELECT position_id, qty, side FROM te_positions WHERE user_id = $1',
    [user]
  );
  ...
  console.log(
    'Spawning Worker A and Worker B concurrently to test race condition with artificial delay...'
  );
  ...
  const posResAfter = await pool.query(
    'SELECT position_id, realized_pnl, qty, side, avg_entry_price FROM te_positions WHERE user_id = $1',
    [user]
  );
