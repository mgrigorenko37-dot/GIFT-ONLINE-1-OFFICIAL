...
async function main() {
  ...
  await pool.query(
    'INSERT INTO te_balances (user_id, currency, available_balance, locked_balance, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6)',
    [user, 'STARS', 500, 0, Date.now(), Date.now()]
  );
  ...
  const posRes = await pool.query(
    'SELECT position_id, instrument_key, realized_pnl, qty, side, avg_entry_price FROM te_positions WHERE user_id = $1',
    [user]
  );
  let tonPos, starsPos;
  ...
  await pool.query(
    'DELETE FROM te_funding_payments WHERE funding_timestamp = $1 AND user_id = $2',
    [ts, user]
  );
  ...
  const runWorker = (name: string) => {
    return new Promise((resolve) => {
      const proc = spawn('npx', ['tsx', 'multi_worker_2.ts', name, String(ts), user], {
        env: process.env,
      });
  ...
  const payTon = await pool.query(
    'SELECT COUNT(*) as c FROM te_funding_payments WHERE position_id = $1 AND funding_timestamp = $2',
    [tonPos.position_id, ts]
  );
  const payStars = await pool.query(
    'SELECT COUNT(*) as c FROM te_funding_payments WHERE position_id = $1 AND funding_timestamp = $2',
    [starsPos.position_id, ts]
  );
  ...
  // 3. Outbox and Ledger
  const outboxTon = await pool.query(
    `SELECT COUNT(*) as c FROM te_outbox_events WHERE user_id = $1 AND event_type = 'ledgerUpdated' AND payload LIKE '%"currency":"TON"%' AND payload LIKE '%FUNDING%'`,
    [user]
  );
  const outboxStars = await pool.query(
    `SELECT COUNT(*) as c FROM te_outbox_events WHERE user_id = $1 AND event_type = 'ledgerUpdated' AND payload LIKE '%"currency":"STARS"%' AND payload LIKE '%FUNDING%'`,
    [user]
  );
  ...
  // 4. Invariants
  const posResAfter = await pool.query(
    'SELECT position_id, instrument_key, realized_pnl, qty, side, avg_entry_price FROM te_positions WHERE user_id = $1',
    [user]
  );
  for (const p of posResAfter.rows) {
    if (p.instrument_key === 'TON') {
      console.log(
        `TON Position: qty=${p.qty} side=${p.side} avgEntry=${p.avg_entry_price} realizedPnl=${p.realized_pnl}`
      );
      console.log(
        `  (Matches initial: qty=${tonPos.qty} side=${tonPos.side} avgEntry=${tonPos.avg_entry_price} realizedPnl=${tonPos.realized_pnl})`
      );
    }
    if (p.instrument_key === 'STARS') {
      console.log(
        `STARS Position: qty=${p.qty} side=${p.side} avgEntry=${p.avg_entry_price} realizedPnl=${p.realized_pnl}`
      );
      console.log(
        `  (Matches initial: qty=${starsPos.qty} side=${starsPos.side} avgEntry=${starsPos.avg_entry_price} realizedPnl=${starsPos.realized_pnl})`
      );
    }
  }
