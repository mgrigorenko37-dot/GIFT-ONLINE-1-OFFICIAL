...
async function main() {
  ...
  const payments = await pool.query(
    'SELECT * FROM te_funding_payments WHERE funding_timestamp = $1 AND user_id = $2',
    [ts, user]
  );
  console.log(`\n--- DB VERIFICATION ---`);
  ...
  console.log(
    `Final balance for user (Initial 200 - fee 0.25 - funding 5.00 = 194.75): ${bal.rows[0].available_balance}`
  );
