...
async function main() {
   ...
   console.log(`Funding executed, payments count: ${payments && payments.length > 0 ? payments[0].payments.length : 0}`);
   
   const outboxCheck = await pool.query('SELECT status, id, event_type FROM te_outbox_events WHERE user_id = $1', [user]);
   ...
   let outboxStatus = await pool.query('SELECT status, id, event_type FROM te_outbox_events WHERE user_id = $1', [user]);
   ...
   outboxStatus = await pool.query('SELECT status, id, event_type FROM te_outbox_events WHERE user_id = $1', [user]);
   for(let row of outboxStatus.rows) {
      console.log(` - ID: ${row.id}, Type: ${row.event_type}, Status: ${row.status}`);
   }
