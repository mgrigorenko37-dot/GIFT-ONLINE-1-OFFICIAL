...
async function runLoadTest() {
  ...
  http.get(`http://localhost:${PORT}/api/market/stats?key=pepe_gift:::TON`, (res) => {
  ...
  const payload = JSON.stringify({
    collection_id: 'pepe_gift', price: 10, quantity: 1, event_time: Date.now()
  });
  const sig = crypto.createHmac('sha256', SECRET).update(`${Date.now()}.${payload}`).digest('hex');
  const req = http.request(`http://localhost:${PORT}/api/sales/ingest`, {
    method: 'POST',
    headers: {
